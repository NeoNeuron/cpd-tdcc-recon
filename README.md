...
- Hodgkin-Huxley, via a Brian2-based simulation implemented directly inside
  `57 - 0 + 0 变点检测（四）.ipynb` ("change-point detection (4)") — used to probe a CPD failure case

## What's here

Everything lives at the repo root (there is no longer a `cp_recon/` subdirectory — all scripts, `common.py`,
and `EINet.py` are siblings):

  - `common.py` — shared utilities, **and the CPD algorithm itself**: `TCD`, `TCD_Ftest` (an F-test-based
    change-point detector), `square_windowed_mean`, `square_windowed_Fstats`. Also re-exports `pdif.utils`
    (as `c4u`) and `pdif.pdif.CausalityEstimator`, so any script doing `from common import *` gets both
    transitively. The TDCC half of the pipeline comes entirely from the external `pdif` package (see
  ...
  - `figures/fig1_cpd_tdcc_recon-pipeline.pdf` — the paper's Figure 1 (pipeline schematic); this one is a
    hand-drawn diagram, not generated by a script
  - `fig2.py` … `fig6.py` — the paper's remaining figures (`fig6.py` is the main CPD/TCD_Ftest change-point
    figure, varying coupling heterogeneity)
...
- **`fig3.py` needs `schematics.png` and `DDV_SVD.png`** in its working directory. Neither file exists
  anywhere in the original monorepo's git history — they're presumably hand-drawn schematics that were never
  committed. `fig3.py` cannot run until these are sourced from wherever they were originally created.
- **No simulation/connectivity/voltage data is committed.** `gen_conn.py`, `recon_LIF.py`, and the balanced-network
  scripts write into `N4000/`, `N32000/`, and `N4000-2-normal/` (git-ignored subdirectories of the repo root) — these
